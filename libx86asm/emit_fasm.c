/*
 * emit_fasm.c - FASM (Flat Assembler) emitter
 *
 * Copyright (c) 2025
 * BSD Licensed - See COPYING
 *
 * Emits x86 assembly in FASM (Intel syntax) format
 */
...
static void fasm_emit_insn(x86asm_ctx_t *ctx, const char *mnemonic,
                           x86asm_operand_t *ops, int nops,
                           x86asm_prefix_t prefix);
static void fasm_emit_directive(x86asm_ctx_t *ctx, const char *name,
                                const char *value);
static void fasm_emit_label(x86asm_ctx_t *ctx, const char *name, int global);
static void fasm_emit_segment(x86asm_ctx_t *ctx, x86asm_segment_t seg,
                              const char *name);
static void fasm_emit_data(x86asm_ctx_t *ctx, x86asm_datasize_t size,
                           const void *data, size_t count);
static void fasm_emit_comment(x86asm_ctx_t *ctx, const char *text);
static void fasm_emit_align(x86asm_ctx_t *ctx, int alignment);

/* Operations table for FASM */
const x86asm_ops_t x86asm_ops_fasm = {
    fasm_emit_insn,
    fasm_emit_directive,
    fasm_emit_label,
    fasm_emit_segment,
...
/*
 * Get size specifier for FASM
 */
static const char *
fasm_size_spec(int size)
{
    switch (size) {
    case 8:   return "byte";
    case 16:  return "word";
...
/*
 * Format an operand for FASM (Intel syntax: dest, source)
 */
static void
fasm_format_operand(x86asm_ctx_t *ctx, x86asm_operand_t *op, char *buf, size_t bufsize)
{
    const char *regname;

    switch (op->type) {
    case OP_REG:
        regname = x86asm_reg_name_internal(ASM_FMT_FASM, op->u.reg, ctx->bits);
    ...
    case OP_MEM:
        ...
        if (op->size > 0) {
            snprintf(buf, bufsize, "%s ", fasm_size_spec(op->size));
        } else {
            buf[0] = '\0';
        }
        ...
        if (op->u.mem.base != REG_NONE) {
            regname = x86asm_reg_name_internal(ASM_FMT_FASM, op->u.mem.base, ctx->bits);
            if (op->u.mem.symbol)
                strcat(buf, " + ");
            strcat(buf, regname);
        }
        ...
        if (op->u.mem.index != REG_NONE) {
            const char *idx_name = x86asm_reg_name_internal(ASM_FMT_FASM, op->u.mem.index, ctx->bits);
 ...
 */
static void
fasm_emit_insn(x86asm_ctx_t *ctx, const char *mnemonic,
               x86asm_operand_t *ops, int nops, x86asm_prefix_t prefix)
{
    ...
    if (nops > 0) {
        for (i = 0; i < nops && i < 3; i++) {
            fasm_format_operand(ctx, &ops[i], op_bufs[i], sizeof(op_bufs[i]));
        }
 ...
 */
static void
fasm_emit_directive(x86asm_ctx_t *ctx, const char *name, const char *value)
{
    if (value)
 ...
 */
static void
fasm_emit_label(x86asm_ctx_t *ctx, const char *name, int global)
{
    if (global)
 ...
 */
static void
fasm_emit_segment(x86asm_ctx_t *ctx, x86asm_segment_t seg, const char *name)
{
    const char *seg_name;
 ...
 */
static void
fasm_emit_data(x86asm_ctx_t *ctx, x86asm_datasize_t size,
               const void *data, size_t count)
{
 ...
 */
static void
fasm_emit_comment(x86asm_ctx_t *ctx, const char *text)
{
    fprintf(ctx->output, "\t; %s\n", text);
}
 ...
 */
static void
fasm_emit_align(x86asm_ctx_t *ctx, int alignment)
{
