 ...
 *
 * Driver program for OCaml compiler
...
#define TYPE_OCAML          1   /* .ml */
#define TYPE_OCAML_IFACE    2   /* .mli */
#define TYPE_ASSEMBLY       3   /* .s */
#define TYPE_OBJECT         4   /* .o */

/* Program paths */
static char *ocom = LIBEXECDIR "/ocom";     /* OCaml compiler */
...
/*
 * Compile OCaml source to assembly
 */
static int
compile_ocaml(const char *infile, const char *outfile)
{
	char *argv[50];
	int argc = 0;
	...
	if (optimization_level > 0) {
		argv[argc++] = "-O";
		char opt_buf[16];
		sprintf(opt_buf, "%d", optimization_level);
...
process_file(const char *file)
{
	...
	case TYPE_OCAML:
		...
		ret = compile_ocaml(file, asmfile);
		if (ret != 0)
	...
	case TYPE_OCAML_IFACE:
		/* Interface files are not compiled directly */
		warning("interface file '%s' is not compiled", file);
		return NULL;
...
usage(void)
{
	printf("Usage: ocaml [options] file...\n");
	printf("Options:\n");
	...
	printf("\nOCaml Compiler for PCC\n");
	printf("Supports OCaml 4.x/5.x source code compilation to native code\n");
	exit(0);
}
