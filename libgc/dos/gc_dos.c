 ...
 * Garbage Collector Implementation - DOS
 * Core mark-and-sweep with ARC support (single-threaded)
 */
...
/* No locking needed for single-threaded DOS */
#define GC_LOCK()
#define GC_UNLOCK()
...
int gc_init(const gc_config_t *config) {
	...
	if (config) {
		...
		/* Default configuration - conservative for DOS */
		gc_state.config.mode = GC_MODE_MARK_SWEEP;
		gc_state.config.precision = GC_CONSERVATIVE;
		gc_state.config.initial_heap_size = 64 * 1024;     /* 64KB */
		gc_state.config.max_heap_size = 512 * 1024;        /* 512KB (conservative for DOS) */
		gc_state.config.num_generations = 2;
...
void gc_collect(void) {
	...
	clock_t end = clock();
	uint64_t elapsed_us = ((uint64_t)(end - start) * 1000000) / CLOCKS_PER_SEC;
...
void gc_print_stats(void) {
	...
	printf("  Total allocated: %lu bytes\n", (unsigned long)gc_state.stats.total_allocated);
	printf("  Total freed: %lu bytes\n", (unsigned long)gc_state.stats.total_freed);
	printf("  Current heap: %lu bytes\n", (unsigned long)gc_state.stats.current_heap_size);
	printf("  Number of objects: %lu\n", (unsigned long)gc_state.stats.num_objects);
	printf("  Collections: %lu\n", (unsigned long)gc_state.stats.num_collections);
	printf("  Total GC time: %lu us\n", (unsigned long)gc_state.stats.total_gc_time_us);
}
