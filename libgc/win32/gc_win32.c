 ...
 * Garbage Collector Implementation - Windows
 * Core mark-and-sweep with ARC support using Windows threading
 */
...
#define GC_LOCK() EnterCriticalSection(&gc_state.gc_mutex)
#define GC_UNLOCK() LeaveCriticalSection(&gc_state.gc_mutex)
...
int gc_init(const gc_config_t *config) {
	...
	InitializeCriticalSection(&gc_state.gc_mutex);
...
void gc_shutdown(void) {
	...
	GC_UNLOCK();
	DeleteCriticalSection(&gc_state.gc_mutex);
}
...
void gc_collect(void) {
	...
	uint64_t elapsed_us = ((end.QuadPart - start.QuadPart) * 1000000) / freq.QuadPart;
