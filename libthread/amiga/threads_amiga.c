/*
 * C11 threads implementation for AmigaOS
 * Uses Exec tasks and semaphores
 */
...
#include <errno.h>
...
#ifdef __amigaos4__
  /* AmigaOS 4.x has pthread support */
  #include <pthread.h>
  #include <sched.h>

  /* Use pthread implementation for OS4 */
...
#else
  /* Classic AmigaOS (68k) - cooperative multitasking, no real threads */
  /* Provide stubs similar to DOS */
  ...
  int mtx_init(mtx_t *mtx, int type) { return thrd_success; }
  int mtx_lock(mtx_t *mtx) { return thrd_success; }
  int mtx_timedlock(mtx_t *restrict mtx, const struct timespec *restrict ts) { return thrd_success; }
  int mtx_trylock(mtx_t *mtx) { return thrd_success; }
  int mtx_unlock(mtx_t *mtx) { return thrd_success; }
  void mtx_destroy(mtx_t *mtx) { }

  int cnd_init(cnd_t *cond) { return thrd_success; }
  int cnd_signal(cnd_t *cond) { return thrd_success; }
  int cnd_broadcast(cnd_t *cond) { return thrd_success; }
  int cnd_wait(cnd_t *cond, mtx_t *mtx) { return thrd_error; }
  int cnd_timedwait(cnd_t *restrict cond, mtx_t *restrict mtx,
                    const struct timespec *restrict ts) { return thrd_timedout; }
  void cnd_destroy(cnd_t *cond) { }
