/*
 * C11 threads implementation for Atari TOS/GEM
 * TOS is single-tasking - provide stubs
 * MiNT adds multitasking but we use stubs for simplicity
 */
...
/*
 * Atari TOS doesn't support threading
 * All functions return error codes or provide minimal stubs
 */

int thrd_create(thrd_t *thr, thrd_start_t func, void *arg)
{
    /* Cannot create threads on TOS */
    return thrd_error;
}

int thrd_equal(thrd_t thr0, thrd_t thr1)
{
    /* Only one thread exists on TOS */
    return 1;
}
...
{
    /* Could use GEMDOS Fselect or busy wait */
    return 0;
}
...
{
    /* Simple index-based TLS for TOS */
