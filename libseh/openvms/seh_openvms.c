 ...
 * SEH Runtime Implementation for OpenVMS
 *
 * OpenVMS has its own condition handling mechanism which is similar
 * conceptually to SEH but uses different APIs.
 */
...
/* OpenVMS uses condition handlers */
...
_seh_register(struct _seh_registration *reg, void *handler, void *filter)
{
	...
	/* TODO: Register with VMS condition handling via LIB$ESTABLISH */
}
