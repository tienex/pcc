 ...
 *
 * Driver program for Pascal compiler
...
#define TYPE_PASCAL         1   /* .pas, .p */
#define TYPE_ASSEMBLY       2   /* .s */
#define TYPE_OBJECT         3   /* .o */

/* Program paths */
static char *pcom = LIBEXECDIR "/pcom";     /* Pascal compiler */
...
static int stop_after_pcom = 0;
static char *output_file = NULL;
static char *dialect = "iso";
...
file_type(const char *filename)
{
	...
	if (strcmp(ext, ".pas") == 0 || strcmp(ext, ".p") == 0 ||
	    strcmp(ext, ".pp") == 0)
		return TYPE_PASCAL;
...
/*
 * Compile Pascal source to assembly
 */
static int
compile_pascal(const char *infile, const char *outfile)
{
	char *argv[50];
	int argc = 0;
...
process_file(const char *file)
{
	...
	case TYPE_PASCAL:
		...
		ret = compile_pascal(file, asmfile);
		if (ret != 0)
...
usage(void)
{
	printf("Usage: pascal [options] file...\n");
	printf("Options:\n");
	printf("  -o <file>     Place output in <file>\n");
	printf("  -c            Compile to object files, don't link\n");
	printf("  -S            Compile to assembly, don't assemble\n");
	printf("  -d <dialect>  Set Pascal dialect (iso, borland, delphi, freepascal, etc.)\n");
	printf("  -L<dir>       Add directory to library search path\n");
	printf("  -l<library>   Link with library\n");
	printf("  -v            Verbose output\n");
	printf("  -h, --help    Show this help\n");
	printf("\nDialects:\n");
	printf("  iso           ISO 7185 Standard Pascal (default)\n");
	printf("  extended      ISO 10206 Extended Pascal\n");
	printf("  microsoft     Microsoft Pascal 4.0\n");
	printf("  clascal       Apple Clascal (Object Pascal)\n");
	printf("  macpascal     Apple MacPascal / MPW Pascal\n");
	printf("  borland       Borland Pascal / Turbo Pascal 7.0\n");
	printf("  delphi        Delphi Object Pascal\n");
	printf("  freepascal    Free Pascal (default mode)\n");
	exit(0);
}
