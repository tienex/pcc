 ...
 *
 * Driver program for PBC (Portable BASIC Compiler)
 * Orchestrates the compilation pipeline similar to pascal driver
 */
...
#define TYPE_BASIC          1   /* .bas, .basic */
#define TYPE_ASSEMBLY       2   /* .s */
#define TYPE_OBJECT         3   /* .o */

/* Program paths */
static char *pbc = LIBEXECDIR "/pbc";      /* BASIC compiler */
...
static int stop_after_pbc = 0;
static char *output_file = NULL;
static char *dialect = "gwbasic";
...
file_type(const char *filename)
{
	...
	if (strcmp(ext, ".bas") == 0 || strcmp(ext, ".BAS") == 0 ||
	    strcmp(ext, ".basic") == 0 || strcmp(ext, ".BASIC") == 0)
		return TYPE_BASIC;
...
/*
 * Compile BASIC source to assembly
 */
static int
compile_basic(const char *infile, const char *outfile)
{
	char *argv[50];
	int argc = 0;
...
process_file(const char *file)
{
	...
	case TYPE_BASIC:
		...
		ret = compile_basic(file, asmfile);
		if (ret != 0)
...
usage(void)
{
	...
	printf("\nDialects:\n");
	printf("  gwbasic       GW-BASIC (default)\n");
	printf("  turbo         Turbo BASIC\n");
	printf("  vbdos         Visual Basic for DOS\n");
	printf("  pds           Microsoft BASIC PDS 7.1\n");
