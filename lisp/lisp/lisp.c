 ...
 *
 * Driver program for Common LISP compiler
 * Orchestrates the compilation pipeline similar to pascal/pascal/pascal.c
 */
...
#define TYPE_LISP           1   /* .lisp, .lsp, .cl */
#define TYPE_ASSEMBLY       2   /* .s */
#define TYPE_OBJECT         3   /* .o */

/* Program paths */
static char *lcom = LIBEXECDIR "/lcom";     /* LISP compiler */
...
file_type(const char *filename)
{
	...
	if (strcmp(ext, ".lisp") == 0 || strcmp(ext, ".lsp") == 0 ||
	    strcmp(ext, ".cl") == 0 || strcmp(ext, ".l") == 0)
		return TYPE_LISP;
...
/*
 * Compile LISP source to assembly
 */
static int
compile_lisp(const char *infile, const char *outfile)
{
	char *argv[50];
	int argc = 0;
...
usage(void)
{
	printf("Usage: lisp [options] file...\n");
	printf("Options:\n");
	...
	printf("  .lisp, .lsp, .cl, .l  - Common LISP source files\n");
	exit(0);
}
